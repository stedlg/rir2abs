function [J_ransac_f,best_S] = build_J_ransac_absolute(f,N_iter,Pi,X,H,threshold_ransac,threshold_Pi,natural_score,K)
...
for n = 1:1:N_iter
    ...
    for k = 1 : 1 : size(H,1)
        ...
        i_inliers_k = find(abs((X(f,struct_Pi(k).cols)-v_k*H(k,struct_Pi(k).cols))) < threshold_ransac);
